...
rep = 1
num = 1
write_trjconv("../rsc/struct.tpr", "../rsc/traj.xtc",
              "nobackup", pbc="whole", ur="tric")
print()
print(np.min(timeit.repeat(lambda: write_traj("../rsc/struct.tpr",
      "../rsc/traj.xtc", transform=NoTransform), number=num, repeat=rep))/num)
print(np.min(timeit.repeat(lambda: write_traj(
    "../rsc/struct.tpr", "../rsc/traj.xtc"), number=num, repeat=rep))/num)
print(np.min(timeit.repeat(lambda: write_trjconv("../rsc/struct.tpr",
      "../rsc/traj.xtc", "nobackup", pbc="whole", ur="tric"), number=num, repeat=rep))/num)
...
u = mda.Universe("../rsc/struct.tpr", "../rsc/traj.xtc")
sel = u.select_atoms("protein")
print(np.min(timeit.repeat(lambda: write_transform(
    sel, notransform), number=num, repeat=rep))/num)

print("cython protein")
u = mda.Universe("../rsc/struct.tpr", "../rsc/traj.xtc")
sel = u.select_atoms("protein")
transform = transformations.Unwrapper(sel, initsetup=True)
print(np.min(timeit.repeat(lambda: write_transform(
    sel, transform), number=num, repeat=rep))/num)
...
bonds1 = transformations.Unwrapper(sel, initsetup=True).bonds
...
u = mda.Universe("../rsc/struct.tpr", "../rsc/struct.gro")
sel = u.select_atoms("protein")
print(timeit.timeit(lambda: transformations.Unwrapper(sel, initsetup=True), number=1))
u = mda.Universe("../rsc/struct.tpr", "../rsc/struct.gro")
sel = u.select_atoms("protein")
...
print(len(frags1), len(frags2), [(len(f1), len(f2))
      for f1, f2 in zip(frags1, frags2)])
print([np.array_equal(f1, f2) for f1, f2 in zip(frags1, frags2)])

u = mda.Universe("../rsc/struct.tpr", "../rsc/struct.gro")
sel = u.select_atoms("protein")
print("cython    ", timeit.timeit(
    lambda: transformations.MolWrapper(sel), number=1))
u = mda.Universe("../rsc/struct.tpr", "../rsc/struct.gro")
sel = u.select_atoms("protein")
print("MDAnalysis", timeit.timeit(
    lambda: numba_transform.MolWrapper_mdafrags(sel), number=1))
u = mda.Universe("../rsc/struct.tpr", "../rsc/struct.gro")
sel = u.select_atoms("protein")
print("Python    ", timeit.timeit(
    lambda: numba_transform.MolWrapper(sel), number=1))
...
u = mda.Universe("../rsc/struct.tpr", "../rsc/struct.gro")
sel = u.select_atoms("all")
print(np.min(timeit.repeat(lambda: transformations.Unwrapper(
    sel, initsetup=True), number=num, repeat=rep))/num)

print("MolWrapper whole")
u = mda.Universe("../rsc/struct.tpr", "../rsc/struct.gro")
sel = u.select_atoms("all")
print(np.min(timeit.repeat(lambda: transformations.MolWrapper(
    sel, initsetup=True), number=num, repeat=rep))/num)
...
print("cython prot")
print(np.min(timeit.repeat(lambda: write_traj("../rsc/struct.tpr",
      "../rsc/traj.xtc", "protein"), number=num, repeat=rep))/num)

print("trjconv prot")
print(np.min(timeit.repeat(lambda: write_trjconv("../rsc/struct.tpr", "../rsc/traj.xtc",
      "nobackup", input=b"Protein\n", pbc="whole", ur="tric"), number=num, repeat=rep))/num)
